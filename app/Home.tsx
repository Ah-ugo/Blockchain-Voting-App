    ...
    switch (activeTab) {
      case "Ongoing":
        return polls
          ...
          .filter((poll) => poll.title.toLowerCase().includes(lowerCaseQuery));
      case "Voted":
        return votingHistory
          .map((history) => polls.find((poll) => poll._id === history.poll_id))
          .filter(
            (poll) => poll && poll.title.toLowerCase().includes(lowerCaseQuery)
          );
      case "Ended":
        return polls
          .filter((poll) => !poll.is_active)
